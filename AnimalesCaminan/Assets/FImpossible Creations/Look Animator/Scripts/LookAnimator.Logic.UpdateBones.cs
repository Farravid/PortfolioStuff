...
namespace FIMSpace.FLook
{
    ...
    public partial class FLookAnimator
    {
            ...
            if (UseBoneOffsetRotation)
            {
                if (DetectZeroKeyframes)
                {
                    for (int i = 0; i < LookBones.Count; i++)
                    {
                        ...
                        if (LookBones[i].lastFinalLocalRotation == LookBones[i].Transform.localRotation)
        ...
        /// <summary>
        /// Resetting animation state of bones when using ultra smoother
        /// </summary>
        private void ResetAnimateBones()
        {
            for (int i = 0; i < LookBones.Count; i++)
            {
                LookBones[i].animatedTargetRotation = LookBones[i].Transform.rotation;
                //LookBones[i].animatedRotationUltra = LookBones[i].animatedTargetRotation;
                LookBones[i].finalRotation = LookBones[i].animatedTargetRotation;
            }
        }
